...
/* El Manijero · panel.js v1.1
   Lógica original intacta + visualizaciones de audio corregidas
   Gauges: arcos segmentados tipo medidor analógico
   Knob: touch corregido con coordenadas relativas al elemento
   ─────────────────────────────────────────────────────────────────────── */
...
// ══════════════════════════════════════════════════════════════════════════
// GAUGE EN ARCO SEGMENTADO — como en la imagen de referencia
// ══════════════════════════════════════════════════════════════════════════

/*
  Dibuja un arco de segmentos rectangulares (tipo velocímetro analógico).
  - canvas: elemento canvas
  - pct: 0–100
  - colors: array de colores por zona (de izq a der del arco)
  - El arco va de 210° a 330° (apertura abajo, como un velocímetro)
*/
function drawGaugeArc(canvasId, pct, colors) {
  ...
  var cx       = W / 2;
  var cy       = H * 0.88;          // centro bajo para que el arco abra hacia arriba
  var r        = Math.min(W, H * 2) * 0.44;
  var segments = 24;
  var startDeg = 210;               // ángulo inicio (izquierda)
  var endDeg   = 330;               // ángulo fin   (derecha)
  var totalArc = endDeg - startDeg; // 120°
  var gap      = 3.5;               // grados de hueco entre segmentos
  var segArc   = (totalArc / segments) - gap;
  var filled   = Math.round((pct / 100) * segments);

  for (var i = 0; i < segments; i++) {
    var angleStart = (startDeg + i * (totalArc / segments)) * Math.PI / 180;
    var angleEnd   = angleStart + (segArc * Math.PI / 180);

    // Color según zona y si está activo
    var zonePct = i / segments;
    var color;
    if (colors.length === 1) {
      color = i < filled ? colors[0] : 'rgba(255,255,255,0.07)';
    } else {
      // Gradiente por zonas
      if (zonePct < 0.4)       color = i < filled ? colors[0] : 'rgba(255,255,255,0.07)';
      else if (zonePct < 0.75) color = i < filled ? colors[1] : 'rgba(255,255,255,0.07)';
      else                     color = i < filled ? colors[2] : 'rgba(255,255,255,0.07)';
    }

    var segW = r * 0.22; // ancho del segmento

    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(angleStart + (segArc * Math.PI / 180) / 2);
    ctx.fillStyle = color;
    ctx.beginPath();
    // Rectángulo centrado en el radio, orientado radialmente
    var inner = r - segW;
    var outer = r;
    ctx.fillRect(-segW / 2, -outer, segW, segW);
    ctx.restore();

    // Alternativo más limpio: arco con strokeStyle grueso
  }

  // Dibujo limpio con arcos por segmento
  ctx.clearRect(0, 0, W, H);
  var lineW = r * 0.22;
...
// Paletas por gauge (igual que referencia: energía rojo, densidad dorado, fatiga verde, conexión rojo-rosa)
var GAUGE_COLORS = {
  energia:  ['#7A1A1A', '#C4403A', '#E85050'],   // oscuro → rojo
  densidad: ['#5A3A08', '#A8742A', '#C9924A'],   // oscuro → dorado
  fatiga:   ['#0A3A18', '#2A8A4A', '#4CAF7D'],   // oscuro → verde
  conexion: ['#6A0A28', '#B03060', '#E87090'],   // oscuro → rosa
};
...
// ══════════════════════════════════════════════════════════════════════════
// KNOB — drag vertical, funciona en mouse Y touch
// ══════════════════════════════════════════════════════════════════════════
...
function getEventY(e) {
  // Devuelve Y tanto para mouse como para touch
...
function initKnob() {
  ...
  // Touch — passive:false para poder hacer preventDefault y evitar scroll
...
function reproducirTema(index) {
  ...
  if (!videoId) { console.warn('URL inválida:', tema.URL); avanzarTema(); return; }
  detenerTimers();
  renderTemaActual(tema, index);
  renderCola(biblioteca.slice(index + 1, index + 6));
  if (esCortina(tema)) {
    cortinaTimer = setTimeout(function() { avanzarTema(); }, CORTINA_DURACION_SEG * 1000);
  }
  initYouTubePlayer(videoId);
}
function avanzarTema() { if (estadoPanel !== 'playing') return; indexActual++; reproducirTema(indexActual); }
function esCortina(t)  { return (t.Estilo || '').toLowerCase() === 'cortina'; }
...
function renderCola(temas) {
  ...
  if (!lista) return;
  if (!temas.length) { lista.innerHTML = '<div class="q-item"><div class="q-info"><div class="q-track">Cola vacía</div></div></div>'; return; }
...
function actualizarLiveBadge() {
  ...
  if (!b) return;
  if (estadoPanel === 'playing') b.innerHTML = '<div class="live-dot"></div><span> En vivo · reproduciendo</span>';
  else if (estadoPanel === 'paused') b.innerHTML = '<div class="live-dot" style="background:#c9a84c;animation:none"></div><span> En pausa</span>';
  else b.innerHTML = '<div class="live-dot" style="background:#555;animation:none"></div><span> Detenido</span>';
}
...
function actualizarUIPista() {
  ...
  setEl('m-pista-sub', caida >= ABANDONO_UMBRAL_PCT ? '↓ ' + caida + '% · posible abandono' : ref + ' hace ' + ABANDONO_VENTANA_MIN + 'min');
  ...
  ['vol','bass','treble'].forEach(function(id) {
    ...
    s.addEventListener('input', function() {
      ...
      if (id === 'vol') {
        ...
        drawKnob(knobValue);
        if (ytPlayer && ytReady) ytPlayer.setVolume(knobValue);
      }
    });
  });
}
...
console.log('El Manijero panel v1.1 · ¡A bailar!');
